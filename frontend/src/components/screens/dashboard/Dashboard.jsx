...
import { EVENT_ENTITY_CHANGED, subscribeTo, unsubscribeToken } from '../../../util/events';
...
export default () => {
    ...
    const { data: statementData, loading: statementLoading, error: statementError, refetch } = useQuery(GET_STATEMENT, {
        variables: {
            input: {
    ...
    const initialBalance = statementData.statement.initialAccountTotals.reduce((p, c) => p + c.amount, 0)

    return (
        <Grid container spacing={2}>
            <Grid item xs={12} style={{ marginTop: '8px' }}>
                <FilterBar setMonth={handleSetMonth} date={dashboardSettings.fromDate} />
            </Grid>
            <Grid item xs={12}>
                <Grid container spacing={2}>
                    <Grid item sm={12} md>
                        <TransactionsTable transactions={statementData.statement.transactions} initialBalance={initialBalance} onDelete={deleteTransaction} onConfirm={() => { }} />
                    </Grid>
